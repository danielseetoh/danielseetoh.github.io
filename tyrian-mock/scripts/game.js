document.addEventListener("DOMContentLoaded", function(event){
	...
	//initialize own ship
	var ownShipObject = new Projectile((window.innerWidth-ownShipXDimension)/2, window.innerHeight-ownShipYDimension-20, 50, 50, 7, 7, true);
	...
	function dropPowerUp(){
		if(Math.random()*1000>999){
			powerUpArray.push(new Projectile(Math.random()*canvas.width-powerUpXDimension, 20+Math.random()*10, powerUpXDimension, powerUpYDimension, powerUpXSpeed, powerUpYSpeed, true));
		}
	}

	//spawn enemy boss or fires shots if it exists
	function EnemyBoss(){
		if (!enemyBossExist){
			if(Math.random()*10000>9990){
				enemyBossExist = true;
				enemyBossHealth = 200;
				enemyBoss = new Projectile(Math.max(Math.random()*canvas.width-enemyBossXDimension,0), 10+Math.random()*5, enemyBossXDimension, enemyBossYDimension, enemyBossXSpeed, enemyBossYSpeed, true);
				ctx.drawImage(enemyBoss1, enemyBoss.x, enemyBoss.y, enemyBoss.xDimension, enemyBoss.yDimension);
			}
		} else {
			drawEnemyBossHealth();
			enemyBoss.x += enemyBoss.xSpeed;
			ctx.drawImage(enemyBoss1, enemyBoss.x, enemyBoss.y, enemyBoss.xDimension, enemyBoss.yDimension);
			fireEnemyBossShots();
		}
	}
	...
	function fireEnemyBossShots(){
		if(Math.random()*1000>960){
			enemyShotArray.push(new Projectile(enemyBoss.x+(enemyBoss.xDimension-enemyShotXDimension)/3, enemyBoss.y + enemyBossYDimension, enemyShotXDimension, enemyShotYDimension, enemyShotXSpeed, enemyShotYSpeed, true))
			enemyShotArray.push(new Projectile(enemyBoss.x+(enemyBoss.xDimension-enemyShotXDimension)*2/3, enemyBoss.y + enemyBossYDimension, enemyShotXDimension, enemyShotYDimension, enemyShotXSpeed, enemyShotYSpeed, true))
		}
	}
	...
	function drawOwnShip(){
		...
		if (ownShipObject.exist){
			if (leftPressed){
				ctx.drawImage(ownShipLeft, ownShipObject.x, ownShipObject.y, ownShipObject.xDimension, ownShipObject.yDimension);
			} else if (rightPressed) {
				ctx.drawImage(ownShipRight, ownShipObject.x, ownShipObject.y, ownShipObject.xDimension, ownShipObject.yDimension);
			} else {
				ctx.drawImage(ownShip, ownShipObject.x, ownShipObject.y, ownShipObject.xDimension, ownShipObject.yDimension);
			}
		} else {
			//alert('GAME OVER');	
			drawGameOver();
	...
	function ownShipAction(){
		if(rightPressed && ownShipObject.x < canvas.width - ownShipObject.xDimension){
		...
		if(downPressed && ownShipObject.y < canvas.height - ownShipObject.yDimension){
			ownShipObject.y += ownShipObject.ySpeed;
		}
		if(spacebarPressed){
			if (!shotAwaiting){
	...
	function fireShot(){
		ownShotArray.push(new Projectile(ownShipObject.x+(ownShipObject.xDimension-ownShotXDimension)/2, ownShipObject.y, ownShotXDimension, ownShotYDimension, ownShotXSpeed, ownShotYSpeed, true));
	}
	...
	function drawProjectiles(projectileArray, projectileType){
		...
		for (var i = 0; i < projectileArray.length; i++){
			...
			obj.y += obj.ySpeed;
			obj.x += obj.xSpeed;
			ctx.drawImage(projectileType, obj.x, obj.y, obj.xDimension, obj.yDimension);
		...
		tempArray = []
		for(var i = 0; i < projectileArray.length; i++){
			if (projectileArray[i].exist == true){
	...
	function enemyShipActions(){
		...
		if(enemyBossExist){
			if(Math.floor(Math.random()*1000>995)){
				enemyBoss.xSpeed = -1*enemyBossXSpeed;
			}
		}
	...
	function fireEnemyShots(){
		for (var i = 0; i < enemyShipArray.length; i++){
			if (Math.random()>0.4){
				enemyShotArray.push(new Projectile(enemyShipArray[i].x+(enemyShipArray[i].xDimension-enemyShotXDimension)/2, enemyShipArray[i].y, enemyShotXDimension, enemyShotYDimension, enemyShotXSpeed, enemyShotYSpeed, true))
			}
		}
	}
	...
	function detectCollision(){
		for(var i = 0; i < enemyShipArray.length; i++){
			...
			if(enemyShipArray[i].x <= 0 || enemyShipArray[i].x >= canvas.width-enemyShipXDimension){
		...
		if(enemyBossExist){
			if(enemyBoss.x <= 0 || enemyBoss.x >= canvas.width-enemyBossXDimension){
				enemyBoss.xSpeed *= -1;
			}
	...
	function reduceHealth(damageTaken){
		...
		if (health<=0){
			health = 0;
			explosionArray.push([new Projectile(ownShipObject.x, ownShipObject.y, ownShipObject.xDimension, ownShipObject.yDimension, ownShipObject.xSpeed, ownShipObject.ySpeed, ownShipObject.exist), 0]);
			if (lives>0){
				lives -= 1;
				health = 100;
	...
	function checkCollision(objA, objB){
		if (objA.x+objA.xDimension >= objB.x && objA.x <= objB.x+objB.xDimension && objA.y+objA.yDimension >= objB.y && objA.y <= objB.y+objB.yDimension){
			return true;
		} else {
			return false;
		}
	...
	function drawGameOver(){
		...
		ctx.fillText("GAME OVER", (canvas.width-340)/2, (canvas.height-20)/2);
		ctx.font = "30px Arial";
		ctx.fillStyle = "yellow";
		ctx.fillText("Press r to restart", (canvas.width-260)/2, (canvas.height+100)/2 );
		gameOver = true;
	}
	...
	function animateExplosion(){
		...
		for(var i = 0; i < explosionArray.length; i++){
			...
			ctx.drawImage(explosion, explosionWidth*explosionArray[i][1], 0, explosionWidth, explosionHeight, obj.x+obj.xDimension*0.125, obj.y+obj.yDimension*0.125, 0.75*obj.xDimension, 0.75*obj.yDimension);
	...
	//starts a new game
	function resetGame(){
		ownShipObject = new Projectile((window.innerWidth-ownShipXDimension)/2, window.innerHeight-ownShipYDimension-20, 50, 50, 7, 7, true);
		lives = 3;
		health = 100;
		score = 0;
		gameOver = false;
